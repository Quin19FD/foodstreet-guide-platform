...
export default function VendorForgotPasswordPage() {
  ...
  const requestOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("/api/vendor/password/request-otp", {
        method: "POST",
  ...
  const verifyOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("/api/vendor/password/verify-otp", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  const resetPassword = async () => {
    ...
    try {
      ...
      if (res.ok) {
        router.replace("/vendor/login?reset=1");
        return;
      }
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.right}>
        ...
        <div className={styles.form}>
          ...
          {errorMessage ? (
            <div className={styles.field} role="alert" style={{ color: "#ffb4b4", fontSize: 13, marginTop: 10 }}>
              {errorMessage}
            </div>
          ) : null}
          ...
          <div style={{ marginTop: 14, textAlign: "center" }}>
            <a href="/vendor/login" className={styles.forgotLink}>
              Quay lại đăng nhập
            </a>
          </div>
        </div>
