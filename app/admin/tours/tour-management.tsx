...
import { Eye, EyeOff, Plus, RefreshCw, Search, Upload } from "lucide-react";
    ...
    <div className="space-y-5">
          ...
          <tbody>
            {tours.map((tour) => (
              <tr key={tour.id} className="border-t border-slate-100 align-top">
                ...
                <td className="px-4 py-3 text-slate-700">{tour.durationMinutes ? `${tour.durationMinutes} phút` : "-"}</td>
                <td className="px-4 py-3 text-slate-700">{tour.stops.length}</td>
                <td className="px-4 py-3">
                  {tour.isActive ? (
                    <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700">Đang hiển thị</span>
                  ) : (
                    <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600">Đã ẩn</span>
                  )}
                </td>
                <td className="px-4 py-3 text-slate-700">{formatDate(tour.updatedAt)}</td>
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-2">
                    <button type="button" className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50" onClick={() => void openDetail(tour)}>
                      <Eye className="h-3.5 w-3.5" /> Chi tiết
                    </button>
                    <button type="button" className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50" onClick={() => { setSelectedTour(tour); applyFormFromTour(tour); setEditOpen(true); }}>
                      Sửa
                    </button>
                    {tour.isActive ? (
                      <button type="button" className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 px-2.5 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-100" onClick={() => void hideTour(tour)}>
                        <EyeOff className="h-3.5 w-3.5" /> Ẩn
                      </button>
                    ) : (
                      <button type="button" className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 text-xs font-semibold text-emerald-700 hover:bg-emerald-100" onClick={() => void restoreTour(tour)}>
      ...
      <Modal open={createOpen} title="Tạo tour mới" onClose={() => setCreateOpen(false)}>
        ...
        <div className="mt-4 flex items-center justify-end gap-2">
          <button type="button" className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50" onClick={() => setCreateOpen(false)}>Hủy</button>
          <button type="button" className="rounded-xl bg-orange-500 px-4 py-2 text-sm font-semibold text-white hover:bg-orange-600" onClick={() => void createTour()}>Tạo tour</button>
        </div>
      </Modal>
      ...
      <Modal open={editOpen} title="Sửa tour" onClose={() => setEditOpen(false)}>
        ...
        />
        <div className="mt-4 flex items-center justify-end gap-2">
          <button type="button" className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50" onClick={() => setEditOpen(false)}>Hủy</button>
          <button type="button" className="rounded-xl bg-orange-500 px-4 py-2 text-sm font-semibold text-white hover:bg-orange-600" onClick={() => void saveEdit()}>Lưu thay đổi</button>
        </div>
      </Modal>
                    ...
                    {detailTour.stops.map((stop) => (
                      <tr key={stop.id} className="border-t border-slate-100">
                        ...
                        <td className="px-3 py-2 text-slate-700">
                          {typeof stop.poi.latitude === "number" && typeof stop.poi.longitude === "number"
...
function TourForm({
  ...
  const selectedPoiList = selectedPoiIds.map((id) => poiLookup[id] ?? { id, name: id, category: null });

  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2">
        <div>
          <label htmlFor="tour-name" className="text-xs font-semibold text-slate-600">Tên tour</label>
          <input id="tour-name" value={name} onChange={(event) => onNameChange(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none ring-orange-500 focus:ring-2" />
        </div>
        <div>
          <label htmlFor="tour-duration" className="text-xs font-semibold text-slate-600">Thời lượng (phút)</label>
          <input id="tour-duration" value={durationMinutes} onChange={(event) => onDurationChange(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none ring-orange-500 focus:ring-2" inputMode="numeric" />
        </div>
      </div>
      ...
      <div>
        ...
        <label className="mt-1 flex cursor-pointer items-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-3 text-sm text-slate-700 hover:bg-slate-100">
          ...
          <input type="file" accept="image/*" className="hidden" onChange={(event) => onImageFileChange(event.target.files?.[0] ?? null)} />
        </label>
        {imagePreviewUrl ? <img src={imagePreviewUrl} alt="preview" className="mt-2 h-40 w-full rounded-xl border border-slate-200 object-cover" /> : existingImageUrl ? <img src={existingImageUrl} alt="current" className="mt-2 h-40 w-full rounded-xl border border-slate-200 object-cover" /> : null}
      </div>

      <div>
        <label htmlFor="tour-description" className="text-xs font-semibold text-slate-600">Mô tả</label>
        <textarea id="tour-description" value={description} onChange={(event) => onDescriptionChange(event.target.value)} className="mt-1 min-h-[88px] w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none ring-orange-500 focus:ring-2" />
      </div>

      <div className="rounded-xl border border-slate-200 p-3">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Tìm nhà hàng theo từ khóa</p>
        <div className="mt-2 flex gap-2">
          <input value={restaurantKeyword} onChange={(event) => onRestaurantKeywordChange(event.target.value)} className="h-10 flex-1 rounded-xl border border-slate-200 px-3 text-sm outline-none ring-orange-500 focus:ring-2" placeholder="Nhập từ khóa..." />
          <button type="button" className="rounded-xl bg-slate-800 px-4 text-sm font-semibold text-white hover:bg-slate-900" onClick={onSearchRestaurants} disabled={isPoiSearching}>{isPoiSearching ? "Đang tìm..." : "Tìm"}</button>
        </div>
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-slate-200 p-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Kết quả nhà hàng</p>
          <div className="mt-2 max-h-56 space-y-2 overflow-y-auto pr-1">
            {poiSearchResults.map((poi) => (
              <label key={poi.id} className="flex items-start gap-2 rounded-lg border border-slate-100 px-2 py-2 text-sm">
                <input type="checkbox" className="mt-0.5" checked={selectedPoiIds.includes(poi.id)} onChange={(event) => onTogglePoi(poi.id, event.target.checked)} />
                <span><span className="font-medium text-slate-800">{poi.name}</span><span className="block text-xs text-slate-500">{poi.category ?? "-"}</span></span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 p-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Thứ tự điểm dừng</p>
          <div className="mt-2 space-y-2">
            {selectedPoiList.map((poi, index) => (
              <div key={poi.id} className="flex items-center justify-between rounded-lg border border-slate-100 px-2 py-2">
                <p className="text-sm text-slate-700">#{index + 1} - {poi.name}</p>
                <div className="flex items-center gap-1">
                  <button type="button" className="rounded border border-slate-200 px-2 py-0.5 text-xs text-slate-600" onClick={() => onMovePoi(index, -1)} disabled={index === 0}>Lên</button>
                  <button type="button" className="rounded border border-slate-200 px-2 py-0.5 text-xs text-slate-600" onClick={() => onMovePoi(index, 1)} disabled={index === selectedPoiList.length - 1}>Xuống</button>
                  <button type="button" className="rounded border border-rose-200 bg-rose-50 px-2 py-0.5 text-xs text-rose-700" onClick={() => onRemovePoi(poi.id)}>Xóa</button>
                </div>
              </div>
            ))}
          </div>
